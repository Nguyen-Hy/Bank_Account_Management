...
public class main {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        Date date = new Date();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("z-a-E dd/MM/yyyy");
        ...
        do{
            ...
            System.out.println("\t|| 6.  TÌM TÀI KHOẢN THEO TÊN                       ||");
            System.out.println("\t|| 7.  TÌM TÀI KHOẢN THEO MÃ                        ||");
            System.out.println("\t|| 8.  TÌM TÀI KHOẢN CÓ SỐ DƯ >= 'X';               ||");
            System.out.println("\t|| 9.  XÓA MỘT TÀI KHOẢN (theo mã)                  ||");
            System.out.println("\t|| 10.  GHI DANH SÁCH TÀI KHOẢN RA FIE               ||");
    ...
    private static boolean transfers ( ArrayList<BankInfor> bankInforArrayList ){

        Scanner scanner = new Scanner(System.in);
        Date date = new Date();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("z-a-E dd/MM/yyyy");

        System.out.print("\n\tNhap so tk nguoi chuyen: " );
        var sender = scanner.nextLine();
        var checkIdSender = findAcount_Id(sender, bankInforArrayList);
        if( checkIdSender != null ){

            System.out.print("\n\tNhap so tien muon chuyen: ");
            var money1 = scanner.nextDouble();
            scanner.nextLine();

            System.out.print("\n\tNhap so tk nguoi nhan: " );
            var receiver = scanner.nextLine();
            if( ! sender.equals(receiver) ){
                var checkIdReceiver = findAcount_Id(receiver, bankInforArrayList);
                if( checkIdReceiver != null ){

                    checkIdSender.removeMoneyToAccount(money1);
                    checkIdReceiver.addMoneyToAcount(money1);
                    System.out.println(" " + simpleDateFormat.format(date));
                    ...
                    System.out.println("\n\tKhong tim thay tk co ten \" " + receiver + " \"");
                }
            }else{
                System.out.println("\n\t========== Số tài khoản người chuyển trùng với số tài khoản người nhận ===========");
            }

            //checkSender
        }else{
            System.out.println("\n\tKhong tim thay tk co ten \" " + sender + " \"");
        }

        System.out.println("\n\tChuyen khoan khong thanh cong! ");
        return false;
    }

    private static BankInfor find2Id( ArrayList<BankInfor> bankInforArrayList, String id1, String id2  ){
        for( var i : bankInforArrayList ){
            if(i.getId().equals(id1)){
                for( var j : bankInforArrayList ){
                    if( j.getId().equals(id2)){
                        return j;
                    }
                }
            }
